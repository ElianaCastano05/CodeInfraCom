...
URL_DASHBOARD = 'http://127.0.0.1:1880/dashboard/dashboard'
UDP_IP    = '169.254.87.249'       # Broadcast de la subred
UDP_PORT  = 5000
...
while True:
    for slave in SLAVES:
        try:
            ...
            if len(adu) < 23:
                print(f"  [{slave}] Timeout — solo {len(adu)} bytes recibidos")
                continue

            if not crc_ok(adu):
                print(f"  [{slave}] CRC inválido: {adu.hex(' ')}")
                continue

            # 3. Reenvía el ADU crudo por UDP — sin modificar ningún byte
            #requests.post(adu, (UDP_IP, UDP_PORT))
            #print(f"  [{slave}] TX: {adu.hex(' ')}")
